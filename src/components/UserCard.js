...
const UserItem = ({ user }) => {
  const { name, bfdate, quizdate, allValid } = useRecoilValue(RecordSelectorFamily(user));
  const setUserClicked = useSetRecoilState(isExpandedSelectorFamily(user));
  const [isExpanded, setIsExpanded] = useRecoilState(isExpandedAtomFamily(user));
  ...
  return (
    ...
    >
      <Grid height={56} key={`${name}Grid`} xs={12} sm={8} md={6} lg={4} zeroMinWidth>
        <Accordion
          sx={{
        ...
        >
          ...
          >
            <ListItemText
              ...
              secondary={bfdate === null ? '-' : dayjs(bfdate).format('DD MMM YY')}
          ...
          >
            <ListItemText
              ...
              primary="Quiz"
              primaryTypographyProps={{ noWrap: true }}
              secondary={quizdate === null ? '-' : dayjs(quizdate).format('DD MMM YY')}
              secondaryTypographyProps={{ noWrap: true }}
            />
          </ListItemButton>
        </Accordion>
